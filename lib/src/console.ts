...
> {
    ...
    >(
        ...
        const config: ConsoleConfig<TemplateLog, TemplateError, TemplateWarn, TemplateInfo> = deepMerge({}, this.config, options)
        const methodValue: KeysValueTemplateMethods<TemplateLog, TemplateError, TemplateWarn, TemplateInfo> = deepMerge({}, this.methodsValue, methodsValue)
    ...
    log<T extends string = TemplateLog>(message?: any, template: { template?: T } | null = {}, keysValues: KeysInput<T> | null = {}) {
        const { config: _config, methodsValue } = this.getConfig({ templates: { ...(template?.template && { log: template.template }) } }, { log: keysValues || {} })
    ...
    warn<T extends string = TemplateWarn>(message?: any, template: { template?: T } | null = {}, keysValues: KeysInput<T> | null = {}) {
        const { config: _config, methodsValue } = this.getConfig({ templates: { ...(template?.template && { warn: template.template }) } }, { warn: keysValues || {} })
    ...
    error<T extends string = TemplateError>(message?: any, template: { template?: T } | null = {}, keysValues: KeysInput<T> | null = {}) {
        const { config: _config, methodsValue } = this.getConfig({ templates: { ...(template?.template && { error: template.template }) } }, { error: keysValues || {} })
    ...
    info<T extends string = TemplateInfo>(message?: any, template: { template?: T } | null = {}, keysValues: KeysInput<T> | null = {}) {
        const { config: _config, methodsValue } = this.getConfig({ templates: { ...(template?.template && { info: template.template }) } }, { info: keysValues || {} })
